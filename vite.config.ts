    ...
    plugins: [
        {
            ...
            transform(src, id) {
                ...
                return src
                    ...
                    .replace(/(?<=<[a-z]+)\s+/, ' ') // remove unnecessary whitespace in html tags (after tag name)
                    .replace(/(?<=")\s+(?=>)/, '') // remove unnecessary whitespace in html tags (end of tag)
                    .replace(/\s+(?=\/>)/, '') // remove unnecessary whitespace in self-closing html tags
