...
export default function (ctx: Context): PluginOption {
    ...
    const replaceViteCSSCommentsPlugin = createPlugin(
        'post-build-replace-vite-css-comments',
        {
            enforce: 'post',
            /**
             * Replaces CSS comments generated by vite in the dist script.
             * @param _ - The output options (unused).
             * @param bundle - The bundle object containing chunks and assets.
             * @throws {Error} if the generated userscript could not be found.
             */
            generateBundle(_, bundle) {
                ...
                if (!scriptFile) {
                    throw new Error(
                        'Could not find the dist script chunk/file. How should I replace Vite CSS comments?'
                    );
                }

                const regex = /\/\*\$vite\$:\d+\*\/\s*/gm;

                if (scriptFile.type === 'chunk') {
                    scriptFile.code = scriptFile.code.replace(regex, '');
                } else {
                    scriptFile.source = scriptFile.source
                        .toString()
                        .replace(regex, '');
    ...
    return [
        lintAndFormatPlugin,
        replaceRequiresPlugin,
        replaceViteCSSCommentsPlugin,
    ];
}
