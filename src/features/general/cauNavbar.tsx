...
const reload = async () => {
	...
	await ready();
	Array.from(document.getElementsByClassName('localnavbarplus')).forEach((el) => {
		el.querySelector<HTMLAnchorElement>('a')?.classList.add(globalStyle.noExternalLinkIcon);
	...
	page.addEventListener('scroll', () => {
		if (!ticking) {
			window.requestAnimationFrame(() => {
				...
				const currentScrollDown = currentScrollPosition > previousScrollPosition;
				if (scrollDown != currentScrollDown) {
					localNavbarPlusDiv.classList.toggle(styles.scrollDown, currentScrollDown);
